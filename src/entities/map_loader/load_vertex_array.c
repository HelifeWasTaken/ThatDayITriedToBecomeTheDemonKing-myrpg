    ...
    struct vertex_loader_data *pos, sfTexture *tileset)
{
    struct tile_id_rot id_rot =
        get_real_tile_id_and_rotation(pos->data[pos->x + pos->y * size->mapx]);
    id_rot.id--;
    ...
    sfVertex *quad = sfVertexArray_getVertex(*vertex,
        (pos->x + pos->y * size->mapx) * 4);

    quad[0].position = VEC2F(pos->x * size->tilex, pos->y * size->tiley);
    quad[1].position = VEC2F((pos->x + 1) * size->tilex, pos->y * size->tiley);
    quad[2].position = VEC2F((pos->x + 1) * size->tilex, (pos->y + 1) * size->tiley);
    quad[3].position = VEC2F(pos->x * size->tilex, (pos->y + 1) * size->tiley);
    if (id_rot.id == -1)
        return;
