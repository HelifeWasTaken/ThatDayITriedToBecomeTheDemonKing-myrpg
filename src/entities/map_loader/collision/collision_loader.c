...
static bool load_collision_layer_vtx(game_t *game, ig_layer_t *layer,
    layer_collision_data_t *data)
{
    ...
    sfVertexArray_resize(data->vtx, layer->height * layer->width * 4);
    for (usize_t i = 0; i < layer->width; i++) {
        for (usize_t j = 0; layer->height; j++) {
    ...
    layer_collision_data_t *data, usize_t i)
{
    ...
    for (usize_t i = 0; i < data->size; i++) {
        if (layer->data.data[i] == true)
            data->map[i] = true;
    }
    return (load_collision_layer_vtx(data, layer, data));
}
    ...
    layer_collision_data_t *data)
{
    ...
    for (; i < map->layers->size; i++) {
        if (estrcmp("collision", map->layers->data[i].name) == 0) {
            if (load_collision_layer_data(game,
